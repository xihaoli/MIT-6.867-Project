...
### Final model: based on at least 5 apperance in Linglin's LASSO logistic
# Xihao Li
...
lasso.list <- lasso_en$name[!is.na(lasso_en$lasso) & lasso_en$lasso >= 5][-1] # Use genes that are selected at least 5 times from lasso out of 10 runs
#en.list <- lasso_en$name[!is.na(lasso_en$en) & lasso_en$en >= 5][-1]

bcr_train_lasso <- bcr_train[,colnames(bcr_train) %in% lasso.list]
bcr_train_lasso$y <- bcr_train$y
dim(bcr_train_lasso) # [1] 329  56
...
bcr_test_lasso <- bcr_test[,colnames(bcr_test) %in% lasso.list]
bcr_test_lasso$y <- bcr_test$y
dim(bcr_test_lasso) # [1] 164  56

###############
# Tree based methods
###############
library(tree)
tree.single <- tree(y ~ ., data = bcr_train_lasso)
summary(tree.single)

pdf("Tree_single_selected_lasso.pdf",width = 10, height = 6)
plot(tree.single, main = "Single Decision Tree (Using LASSO selected Genes)")
...
tree.pred = predict(tree.single, newdata = bcr_test_lasso, type = "class") # Note: there is randomness here
(test=with(bcr_test_lasso, table(tree.pred, y)))

(test[2,2] + test[1,1])/sum(test) # [1] 0.6402439
...
pdf("Tree_prune_selected_lasso.pdf",width=10,height=6)
plot(prune.train, main = "Pruned Decision Tree (Using LASSO selected Genes)")
text(prune.train, pretty = 0)
dev.off()

tree.prune.pred = predict(prune.train, newdata = bcr_test_lasso, type = "class")
test=with(bcr_test_lasso, table(tree.prune.pred, y))
...
set.seed(1)
time <- proc.time()
rf.train = randomForest(y ~ ., data = bcr_train_lasso, ntree = 500)
rf.train
# random forest performance (note: not fixed)
(acc_RF_lasso = sum((predict(rf.train, bcr_test_lasso) == bcr_test_lasso$y))/nrow(bcr_test_lasso)) # [1] 0.7073171
print(proc.time()-time)

####### AUC for RandomForest
library(ROCR)
pdf("RF_breastcancer_selected_lasso_roc.pdf",width=10,height=6)
pred = prediction(as.numeric(predict(rf.train, bcr_test_lasso)), as.numeric(bcr_test_lasso$y))
# calculate probabilities for TPR/FPR for predictions
perf = performance(pred,"tpr","fpr")
(auc = performance(pred,"auc")@y.values[[1]]) # shows calculated AUC for model # [1] 0.7092103
plot(perf,colorize=FALSE, col="black") # plot ROC curve
lines(c(0,1),c(0,1),col = "gray", lty = 4 )
text(0.8,0.2, paste0('AUC=', round(auc,3), '\nACC=', round(acc_RF_lasso,3)))
dev.off()

oob.err = double(ncol(bcr_train_lasso)-1)
test.err = double(ncol(bcr_train_lasso)-1)
for (mtry in 1:(ncol(bcr_train_lasso)-1)) {
  fit = randomForest(y ~ ., data = bcr_train_lasso, mtry = mtry, ntree = 500)
  oob.err[mtry] = fit$err.rate[500,1]
  pred = predict(fit, bcr_test_lasso)
  test.err[mtry] = sum(bcr_test_lasso$y != pred)/nrow(bcr_test_lasso)
  cat(mtry, " ")
}
oob.err
test.err

pdf("RF_breastcancer_selected_lasso.pdf",width=10,height=6)
plot(1:mtry, oob.err, pch = 19, col = c("black"), xlab = "Number of predictors chosen as split candidates",
     type = "b", ylab = "Misclassification Error", main = "Bagging and Random Forest Out-Of-Bag Error Plot (Using LASSO selected Genes)")
legend("topright", legend = c("OOB Error Rate"), pch = 19, col = "black")
abline(v = floor(sqrt(mtry)), col = "blue")
text(floor(sqrt(mtry)), 0.19, "Random 
     Forest", col = "blue")
abline(v = mtry, col = "red")
text(mtry, 0.195, "Bagging", col = "red")
1-test.err[which.min(oob.err)] # [1] 0.7012195
dev.off()
...
set.seed(1)
training.boost <- bcr_train_lasso
...
which.min(mean.cv) # 310


# Use the best number of iteration
time <- proc.time()
best.iter <- which.min(mean.cv) # 310
iter <- 500
training.boost <- bcr_train_lasso
training.boost$y=(as.numeric(training.boost$y)-1.5)*2
dim(training.boost)
testing.boost <- bcr_test_lasso
...
(acc_boost_lasso <- 1-test_error[which.min(mean.cv)]) # [1] 0.7317073


pdf("AdaBoost_breastcancer_selected_lasso_acc.pdf",width=10,height=6)
plot(seq(1,iter), 1-train_error, type="l",main="AdaBoost Accuracy Plot (Using Lasso selected Genes)
     (best iteration = 310 selected by 10-fold CV)",ylim=c(0.5,1), ylab="Accuracy", xlab="Number of boosting iterations", lwd=2)
...
####### AUC for AdaBoost
library(ROCR)
pdf("AdaBoost_breastcancer_selected_lasso_roc.pdf",width=10,height=6)
pred = prediction(f_test_lasso_best.iter, as.numeric(bcr_test_lasso$y))
# calculate probabilities for TPR/FPR for predictions
perf = performance(pred,"tpr","fpr")
(auc = performance(pred,"auc")@y.values[[1]]) # shows calculated AUC for model [1] 0.7583221
plot(perf,colorize=FALSE, col="black") # plot ROC curve
lines(c(0,1),c(0,1),col = "gray", lty = 4 )
text(0.8,0.2, paste0('AUC=', round(auc,3), '\nACC=', round(acc_boost_lasso,3)))
dev.off()
